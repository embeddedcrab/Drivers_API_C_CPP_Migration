 ...
 *
 */
long UART__::init( const unsigned char& initialize )
{
	/* Local Variables */
	const tStUART_Configuartion * p_config_l = ((const tStUART_Channel *) p_handle_)->p_config;
	tStUART_Control * p_control_l = ((tStUART_Channel*) p_handle_)->p_control;
...
{
	/* Local Arguments */
	tStUART_Configuartion * p_config_l = ((tStUART_Channel*)p_handle_)->p_config;
	tStUART_Control * p_control_l = ((tStUART_Channel*) p_handle_)->p_control;
...
long UART__::receive( unsigned char * const p_dest, const unsigned long& length )
{
	...
	if( (0 == p_control_l->state.rx_busy) && (0 == p_control_l->state.rx_lock) )/* 0x18 == (p_control_l->state.state_value & 0x18) */
	{
		...
		if( p_config_l->fifo_cfg.rx_fifo_size != XMC_USIC_CH_FIFO_DISABLED )
		{
			...
			reconfigure_rx_fifo( p_control_l->rx_data_count );
 ...
 *
 */
long UART__::abort( const unsigned char& tx_rx )
{
	/* Local Arguments */
	tStUART_Configuartion * p_config_l = ((tStUART_Channel*)p_handle_)->p_config;
	tStUART_Control * p_control_l = ((tStUART_Channel*) p_handle_)->p_control;
...
eUART_Channel UART__::get_channel( void ) const
{
	/* Local variable */
	XMC_USIC_CH_t * context = ((XMC_USIC_CH_t *)(((const tStUART_Channel *) p_handle_)->p_config->p_channel_context));
	eUART_Channel channel = UART_CHANNEL_0;
...
void UART__::calc_fifo( unsigned long (&fifo_size_arr)[2] )
{
	/* Local variables */
	eUART_Channel channel = get_channel();
...
void UART__::reconfigure_rx_fifo( const unsigned long& data_size ) const
{
	/* Local Variables */
	tStUART_Configuartion * p_config_l = ((tStUART_Channel*)p_handle_)->p_config;
	...
	if( 1 == p_control_l->state.tx_busy )
	{
		...
		else/* Data is fully sent, ensure it and complete the transfer */
		{
			...
			if( 1 == XMC_USIC_CH_TXFIFO_IsEmpty( (XMC_USIC_CH_t *const)p_config_l->p_channel_context ) )
			{
				...
				((tStUART_Channel*)p_handle_)->p_callback->fp_callback( get_channel(), NULL, UART_EVENT_TX_COMPLETE );
				#endif
			}
		}
	}
 ...
 *
 */
void UART__::rx_isr_direct( void )
{
	/* Local Arguments */
	tStUART_Configuartion * p_config_l = ((tStUART_Channel*)p_handle_)->p_config;
	...
	/* Call callback */
	#if( UART_CALLBACK_HANDLER_USED == 1U )
	((tStUART_Channel*)p_handle_)->p_callback->fp_callback( get_channel(), &data_l, UART_EVENT_RX_CHAR );
	#endif
	/* Enable Rx Event */
	XMC_USIC_CH_RXFIFO_EnableEvent( (XMC_USIC_CH_t *)(p_config_l->p_channel_context),
				(unsigned long) ((unsigned long) XMC_USIC_CH_EVENT_STANDARD_RECEIVE |
		...
		if( p_config_l->fifo_cfg.rx_fifo_size != XMC_USIC_CH_FIFO_DISABLED )
		{
			...
			while( 0 == XMC_USIC_CH_RXFIFO_IsEmpty( (XMC_USIC_CH_t *const)p_config_l->p_channel_context ) )
			{
				...
				if( p_control_l->rx_data_index == p_control_l->rx_data_count )
				{
					...
					/* Call callback if available */
					#if( UART_CALLBACK_HANDLER_USED == 1U )
					((tStUART_Channel*)p_handle_)->p_callback->fp_callback( get_channel(), NULL, UART_EVENT_RX_COMPLETE );
					#endif
					/* Break operation */
					break;
				}
			...
			if( p_control_l->rx_data_index < p_control_l->rx_data_count )
			{
				reconfigure_rx_fifo( p_control_l->rx_data_count - p_control_l->rx_data_index );
			}
		}
		else
		{
 ...
 *
 */
void UART__::err_isr_entry( void )
{
	/* Local Arguments */
	tStUART_Configuartion * p_config_l = ((tStUART_Channel*)p_handle_)->p_config;
	/* Local variables */
	...
	for( count_l = 0; count_l < max_val_l; ++count_l )
	{
				...
				(psr_status_l & (unsigned long) uart_event_status_flags[count_l]) )
		{
			...
			/* Call Callback */
			#if( UART_CALLBACK_HANDLER_USED == 1U )
			((tStUART_Channel*)p_handle_)->p_callback->fp_callback( get_channel(), NULL, static_cast<eUART_Event>(count_l) );
			#endif
			break;
		}
	}
