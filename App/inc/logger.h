...
/* Convert Logger API according to interface data type */
#define LOGGER_API( serial )						((UART__ *)serial)
...
public:
	...
	explicit Logger__( void * const p_serial )
					: p_serial_( p_serial ), index_( 0 )
	{
		if( nullptr == p_serial )
		{
			for(  ; ; )
	...
	{
		/* Get Mutex */
		while( UART_STATUS_BUSY == LOGGER_API(p_serial_)->transmit( (const unsigned char *)buffer_, sg_logger_buffer_size ) );
		memset( (void *)buffer_, '\0', sg_logger_buffer_size );
		index_ = 0;
		/* Release Mutex */
		/* Return object reference */
